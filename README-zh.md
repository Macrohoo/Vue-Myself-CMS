...
#### 1210创建账号页面添加完成&&阅读ElementUI组件源码
...
```
使用模块中的mutations、getters、actions时候，要加上模块名，例如使用commint执行mutations时格式：①模块名/模块中的mutations  xxx/setUserInfo`this.$store.commit(“userInfo/setUserInfo”,userInfo)` ②获取属性时同样加上模块名格式：store.state.模块名.模块属性`$store.state.userInfo.userName`
